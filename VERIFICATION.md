...
- [x] `Vec<T>`: invariant `len <= capacity`, initialized prefix ownership,
  ...
  write state. The split branch now has an exact source-ordered operational
  trace as well. Each descending-loop row performs offset load/store, size
  ...
  equal to the existing `insertClassWrites` transformer. This trace is now
  composed into the complete split transaction. Candidate removal, physical
  header reads and shifting, and remainder insertion form one source-ordered
...
- [ ] End-to-end examples compile to Rust with no redundant bounds checks and
  ...
  `scripts/verify-clean.sh` is the hermetic repository gate: it exports only
  tracked files into a temporary checkout, rebuilds the pinned Lean dependency
  graph and full library, runs every Rust test, then checks and compiles all
  four examples plus the TLSF and container standard-library modules. CI runs
  this same command from a GitHub checkout, so stale generated files and cached
  `.olean` artifacts cannot satisfy the gate.
