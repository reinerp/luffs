...
struct Module {
    ...
    tlsf_allocate_physical_models: Vec<TlsfCoalescePhysicalModel>,
    ...
    tlsf_box_drop_ptr_models: Vec<TlsfCoalescePhysicalModel>,
...
fn parse_tlsf_allocate_physical_models(source: &str) -> Vec<TlsfCoalescePhysicalModel> {
    let lines = source.lines().collect::<Vec<_>>();
    let Some(index) = lines
        .iter()
        .position(|line| line.trim().starts_with("fn tlsf_allocate_physical("))
    else {
        return Vec::new();
    };
    let Some(target) = index
    ...
    if required.iter().all(|expected| {
        ...
        vec![TlsfCoalescePhysicalModel {
            name: "tlsf_allocate_physical".to_owned(),
            refines: target.trim().to_owned(),
        }]
    } else {
        Vec::new()
...
fn parse_tlsf_allocate_models(source: &str) -> Vec<TlsfCoalescePhysicalModel> {
    let lines = source.lines().collect::<Vec<_>>();
    let Some(index) = lines
        .iter()
        .position(|line| line.trim().starts_with("fn tlsf_allocate("))
    else {
        return Vec::new();
    };
    let Some(target) = index
    ...
    let mut specifications = vec![
        ...
        (
            ...
            vec![
                ...
                "let end: usize = offset.checked_add(16)?;",
                "if end > pool.len() { return None; }",
                "pool[offset] = value as u8;",
                "pool[offset + 1] = (value >> 8) as u8;",
                "pool[offset + 2] = (value >> 16) as u8;",
                ...
                "pool[offset + 6] = (value >> 48) as u8;",
                "pool[offset + 7] = (value >> 56) as u8;",
                "pool[offset + 8] = (value >> 64) as u8;",
                "pool[offset + 9] = (value >> 72) as u8;",
                "pool[offset + 10] = (value >> 80) as u8;",
                "pool[offset + 11] = (value >> 88) as u8;",
                "pool[offset + 12] = (value >> 96) as u8;",
                "pool[offset + 13] = (value >> 104) as u8;",
                "pool[offset + 14] = (value >> 112) as u8;",
                "pool[offset + 15] = (value >> 120) as u8;",
                "Some(offset)",
            ],
        ),
    ];
...
fn parse_tlsf_box_drop_models(source: &str) -> Vec<TlsfCoalescePhysicalModel> {
    ...
    let Some(index) = lines
        .iter()
        .position(|line| line.trim().starts_with("fn tlsf_box_drop_u8("))
    else {
        return Vec::new();
    };
    let Some(target) = index
    ...
    if required.iter().all(|expected| {
        ...
        vec![TlsfCoalescePhysicalModel {
            name: "tlsf_box_drop_u8".to_owned(),
            refines: target.trim().to_owned(),
        }]
    } else {
        Vec::new()
...
fn parse_tlsf_vec_grow_models(source: &str) -> Vec<TlsfCoalescePhysicalModel> {
    let lines = source.lines().collect::<Vec<_>>();
    let variants = [
        ("tlsf_vec_grow_u8", "len", Vec::<&str>::new()),
        (
            "tlsf_vec_grow_u16",
            "initialized_bytes",
            vec![
    ...
    for (name, copied_len, arithmetic) in variants {
        ...
        required.extend(arithmetic.into_iter().map(str::to_owned));
        required.extend([
            format!("let old_end: usize = old_offset.checked_add({copied_len})?;"),
            "let new_offset: usize = tlsf_allocate(offsets, sizes, is_free, prev_free, second_nonempty, first_nonempty, heads, next, previous, block_count, request)?;".to_owned(),
            format!("let new_end: usize = new_offset.checked_add({copied_len})?;"),
            format!("while cursor < {copied_len} {{"),
            "let source: usize = old_offset.checked_add(cursor)?;".to_owned(),
            "let destination: usize = new_offset.checked_add(cursor)?;".to_owned(),
            "let byte: u8 = pool[source];".to_owned(),
            "pool[destination] = byte;".to_owned(),
            "tlsf_box_drop_u8(offsets, sizes, is_free, prev_free, second_nonempty, first_nonempty, heads, next, previous, block_count, old_offset)?;".to_owned(),
            "Some(new_offset)".to_owned(),
        ]);
        let mut position = 0;
        if required.iter().all(|expected| {
            if let Some(offset) = body[position..].iter().position(|line| line == expected) {
...
fn lean(module: &Module) -> String {
    ...
    if module
        ...
        || !module.tlsf_box_drop_ptr_models.is_empty()
    ...
    if !module.tlsf_insert_models.is_empty()
        ...
        || !module.tlsf_allocate_models.is_empty()
...
{} offsets sizes is_free prev_free block returned_offset returned_bytes\n\n",
    ...
    for model in &module.tlsf_allocate_physical_models {
        out.push_str(&format!(
            "def {}_model (offsets sizes : List Nat) (is_free prev_free : List (Fin 256)) (count block request : Nat) : Option Luffs.Runtime.TLSF.AllocatePhysicalResult :=\n  {} offsets sizes is_free prev_free count block request\n\n",
            model.name, model.refines
        ));
        out.push_str(&format!(
            "theorem {}_refines : {}_model = {} := by rfl\n\n",
    ...
    for model in &module.tlsf_box_drop_models {
        out.push_str(&format!(
            "def {}_model (offsets sizes : List Nat) (is_free prev_free : List (Fin 256)) (count : Nat) (second : List (BitVec 32)) (first : BitVec 64) (heads next previous : List Nat) (returned_offset : Nat) : Option Luffs.Runtime.TLSF.CoalesceClassResult := do\n  if count = 0 ∨ count > offsets.length ∨ count > sizes.length ∨ count > is_free.length then none\n  let block ← tlsf_find_offset_index_model offsets count returned_offset\n  if is_free[block]? ≠ some 0 then none\n  let returned_bytes ← sizes[block]?\n  tlsf_deallocate_composed_model offsets sizes is_free prev_free second first heads next previous count block returned_offset returned_bytes\n\n",
            model.name
        ));
        out.push_str(&format!(
            "theorem {}_refines : {}_model = {} := by\n  unfold {}_model\n  rw [tlsf_find_offset_index_refines, tlsf_deallocate_composed_refines]\n  rfl\n\n",
            model.name, model.name, model.refines, model.name
        ));
    }
    ...
    for model in &module.tlsf_deallocate_models {
        ...
        out.push_str(&format!(
            "theorem {}_refines : {}_model = {} := by\n  unfold {}_model\n  exact tlsf_deallocate_composed_refines\n\n",
            model.name, model.name, model.refines, model.name
        ));
    }
...
mod tests {
    ...
    fn tlsf_runtime_source_has_only_proved_accesses() {
        ...
        assert_eq!(m.tlsf_allocate_physical_models.len(), 1);
        assert_eq!(m.tlsf_allocate_models.len(), 1);
        ...
        assert!(
            generated
                .contains("let block ← tlsf_find_offset_index_model offsets count returned_offset")
        );
        assert!(generated.contains(
            "tlsf_deallocate_composed_model offsets sizes is_free prev_free second first heads next previous count block returned_offset returned_bytes"
        ));
        ...
        assert!(!generated.contains(
            "Option Luffs.Runtime.TLSF.CoalesceClassResult :=\n  Luffs.Runtime.TLSF.deallocateArrays"
        ));
        assert!(generated.contains(
        ...
        assert!(generated.contains(
            "theorem tlsf_vec_grow_u32_refines : tlsf_vec_grow_u32_model = Luffs.Runtime.Containers.vecGrowU32Arrays"
        ));
        assert!(generated.contains(
            "Luffs.Memory.Program.forRange 0 (len * 4)\n        (Luffs.Memory.Program.copyLoopBody oldBase newBase)"
        ));
    ...
    #[test]
    fn tlsf_allocation_refinement_rejects_too_small_remainders() {
        let source = include_str!("../stdlib/tlsf.luffs").replace(
            "let split: bool = remainder_size >= 16;",
            "let split: bool = remainder_size >= 8;",
        );
        let m = parse(&source).unwrap();
    ...
    #[test]
    fn tlsf_box_u128_refinement_rejects_missing_high_byte() {
        let source = include_str!("../stdlib/tlsf.luffs")
            .replace("pool[offset + 15] = (value >> 120) as u8;", "");
        let module = parse(&source).unwrap();
        assert!(
            !module
                .tlsf_box_new_models
    ...
    #[test]
    fn tlsf_box_drop_refinement_rejects_missing_count_update() {
        let source =
    ...
    #[test]
    fn tlsf_box_drop_ptr_refinement_rejects_unvalidated_pointer() {
        let source = include_str!("../stdlib/tlsf.luffs").replace(
            "let offset: usize = tlsf_pointer_to_offset(pool_base, pool_bytes, pointer)?;",
    ...
    #[test]
    fn tlsf_vec_grow_u16_refinement_rejects_missing_byte_multiply() {
        let source = include_str!("../stdlib/tlsf.luffs").replace(
            "let initialized_bytes: usize = len.checked_mul(2)?;",
            "let initialized_bytes: usize = len;",
        );
        let m = parse(&source).unwrap();
        assert!(
            !m.tlsf_vec_grow_models
                .iter()
                .any(|model| model.name == "tlsf_vec_grow_u16")
        );
    }

    #[test]
    fn tlsf_vec_grow_u32_refinement_rejects_wrong_byte_multiply() {
        let source = include_str!("../stdlib/tlsf.luffs").replace(
            "let initialized_bytes: usize = len.checked_mul(4)?;",
            "let initialized_bytes: usize = len.checked_mul(2)?;",
        );
    ...
    fn container_runtime_source_has_only_proved_accesses() {
        ...
        assert!(generated.contains(
            "theorem vec_push_i128_refines : vec_push_i128_model = Luffs.Runtime.Containers.vecPushI128"
        ));
        ...
        assert!(generated.contains("theorem box_load_u8_program_wp"));
        assert!(generated.contains(
            "theorem box_store_u8_refines : box_store_u8_model = Luffs.Runtime.Containers.boxStoreU8"
        ));
        assert!(generated.contains("theorem box_store_u8_program_wp"));
        assert!(generated.contains(
        ...
        assert!(generated.contains(
            "theorem box_load_u64_refines : box_load_u64_model = Luffs.Runtime.Containers.boxLoadU64"
        ));
        assert!(generated.contains("theorem box_load_u64_program_wp"));
        assert!(generated.contains(
            "theorem box_store_u64_refines : box_store_u64_model = Luffs.Runtime.Containers.boxStoreU64"
        ));
        assert!(generated.contains("theorem box_store_u64_program_wp"));
        assert!(generated.contains(
            "theorem box_load_u128_refines : box_load_u128_model = Luffs.Runtime.Containers.boxLoadU128"
        ));
        assert!(generated.contains("theorem box_load_u128_program_wp"));
        assert!(generated.contains(
